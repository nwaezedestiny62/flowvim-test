    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-light px-4">
        <div className="text-center">
          ...
          <a href="/shop" className="text-prim hover:underline text-lg">← Back to Shop</a>
        </div>
      </div>
    );
  }
      ...
      {galleryImages.map((img: string, index: number) => (
        <button
          ...
          className={`relative aspect-square overflow-hidden rounded-2xl border-2 transition-all ${
            selectedImageIndex === index ? 'border-yellow-500 scale-105' : 'border-transparent hover:border-gray-300'
          }`}
        >
                  ...
                  <div>
                    ...
                    <div className="grid gap-4">
                      {features.map((feature: string, index: number) => (
                        <div key={index} className="flex gap-4 rounded-3xl border border-white/50 bg-white/40 p-5 backdrop-blur-xl transition-all hover:-translate-y-1 hover:shadow-xl">
                          <Icon icon="mdi:check-circle" className="flex-shrink-0 text-green-600 mt-0.5" width={28} />
                ...
                <div className="rounded-[28px] md:rounded-[32px] border border-white/50 bg-white/50 backdrop-blur-xl p-6 md:p-7 shadow-xl">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
                    <div className="flex items-center overflow-hidden rounded-2xl border border-gray-200 bg-white w-full sm:w-auto">
                      <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="px-6 py-4 text-2xl hover:bg-gray-100 active:bg-gray-200 transition-colors w-1/3 sm:w-auto">−</button>
                      <span className="px-8 py-4 font-bold text-xl flex-1 text-center border-x">{quantity}</span>
                      <button onClick={() => setQuantity((q) => q + 1)} className="px-6 py-4 text-2xl hover:bg-gray-100 active:bg-gray-200 transition-colors w-1/3 sm:w-auto">+</button>
                    </div>

                    <div className="sm:ml-auto">
                      <p className="text-sm text-gray-500">Total</p>
                      <p className="font-black text-2xl md:text-3xl">₦{(product.price * quantity).toLocaleString()}</p>
                    </div>
                  </div>

                  <button onClick={handleAddToCart} className="w-full rounded-2xl bg-yellow-500 py-5 text-lg font-bold text-black transition-all hover:scale-[1.02] active:scale-[0.98] hover:shadow-[0_10px_40px_rgba(234,179,8,0.4)]">
                    Add {quantity} To Cart
                  </button>

                  <button
                    onClick={() => {
                      const message = `Hi, I'm interested in ${product.title} (Quantity: ${quantity}) - ₦${(product.price * quantity).toLocaleString()}`;
                      window.open(`https://wa.me/2347010930763?text=${encodeURIComponent(message)}`, "_blank");
                    }}
                  ...
                  <Link href="/shop" className="group mt-10 inline-flex items-center gap-2.5 text-[15px] font-medium text-gray-600 hover:text-gray-900 transition-all duration-300 mx-auto">
                    <Icon icon="mdi:arrow-left" width={24} className="transition-transform group-hover:-translate-x-1" />
                    Back to Shop
                  </Link>
                </div>
              </div>
