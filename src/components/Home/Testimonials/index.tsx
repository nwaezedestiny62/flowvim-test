...
const Testimonials: React.FC = () => {
  ...
  const testimonials = content?.testimonials || {
    mainTitle: "Real Results from Real Businesses",
    rating: "4.9",
    reviewsCount: "50+",
    items: []
  };
  ...
  const cardVariants: Variants = { hidden: { opacity: 0, y: 60, scale: 0.97 }, show: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.7, ease: "easeOut" } } };
  const floatInRight: Variants = { hidden: { opacity: 0, x: 80 }, show: { opacity: 1, x: 0, transition: { duration: 0.8, ease: "easeOut" } } };
  const floatInLeft: Variants = { hidden: { opacity: 0, x: -80 }, show: { opacity: 1, x: 0, transition: { duration: 0.8, ease: "easeOut" } } };
    ...
    <section className="overflow-hidden py-14 lg:py-18 xl:py-22 bg-prim-light">
      ...
      >
        <div className="flex flex-col lg:flex-row w-full gap-5 lg:gap-8">
          ...
          {/* IMAGE SIDE - Static for now */}
          <motion.div
            variants={floatInLeft}
            className="relative w-full lg:w-1/2 rounded-3xl overflow-hidden h-[320px] sm:h-[420px] lg:h-[520px]"
          >
            <Image src="/images/testimonials/testimonial-img.jpg" alt="Flowvim client testimonials" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" />
            ...
            <div className="hidden md:flex absolute right-5 bottom-5 bg-dark text-white h-[150px] w-[200px] flex-col justify-between p-4 rounded-2xl z-10">
              <div className="text-5xl font-unbounded">{testimonials.rating}</div>
              <div>
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Icon key={i} icon="material-symbols:star-rounded" width="22" height="22" className="text-yellow-400" />
                  ))}
                </div>
                <p className="text-sm">({testimonials.reviewsCount} Client Reviews)</p>
              </div>
            </div>
          ...
          {/* SWIPER SIDE - Editable Testimonials */}
          <motion.div variants={floatInRight} className="w-full lg:w-1/2">
            <Swiper
              spaceBetween={20}
              slidesPerView={1}
              loop={true}
            ...
            >
              {testimonials.items.map((t: any, index: number) => (
                <SwiperSlide key={index}>
                  ...
                  >
                    <Icon icon="iconoir:quote-solid" width={70} height={70} className="text-dark mb-5" />
                    ...
                    <div className="user flex items-center gap-4 pt-8">
                      ...
                      <div>
                        ...
                        <span className="text-pera-light text-sm">{t.position}</span>
                      </div>
                    </div>
                  </motion.div>
                </SwiperSlide>
